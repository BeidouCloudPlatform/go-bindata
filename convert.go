...
func Translate(c *Config) error {
	...
	if len(c.Arch) > 0 && !strings.Contains(c.Output, c.Arch) {
		c.Output = strings.ReplaceAll(c.Output, ".go", "_"+c.Arch+".go")
	}
	...
	if len(c.Arch) > 0 {
		if _, err = fmt.Fprintf(bfd, "// go:build %s\n// +build %s\n", c.Arch, c.Arch); err != nil {
			return err
		}
	}
